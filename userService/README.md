...
2. 生成模板
```
goctl rpc protoc userService.proto --style goZero --go_out=. --go-grpc_out=. --zrpc_out=.
```
3. 编辑`userService.proto`定义
    1. package 规范
    ```
    # 使用全小写
...
4. 生成rpc文件夹路径
```
goctl rpc proto -src userService.proto -dir . -style goZero
```
